...
import (
	...
	"github.com/kisekivul/graceful/executor"
)
...
func (mp *master) run() error {
	...
	if mp.Config.Fetcher != nil {
		if err := mp.Config.Fetcher.Init(); err != nil {
			mp.warn("fetcher init failed (%s). fetcher disabled.", err)
...
func (mp *master) checkBinary() error {
	...
	if mp.Config.Fetcher != nil {
		if err := executor.Move(tmpBinPath, mp.binPath); err != nil {
			return fmt.Errorf("cannot move binary (%s)", err)
		}
		if err := executor.Move(mp.binPath, tmpBinPath); err != nil {
...
func (mp *master) handleSignal(s os.Signal) {
	...
	if mp.awaitingUSR1 && s == executor.SIGUSR1 {
		mp.debug("signaled, sockets ready")
	...
	if mp.slaveCmd != nil && mp.slaveCmd.Process != nil {
		mp.debug("proxy signal (%s)", s)
		mp.sendSignal(s)
	} else
	//otherwise if not running, kill on CTRL+c
	if s == os.Interrupt {
		mp.debug("interupt with no slave")
		os.Exit(1)
	} else {
		mp.debug("signal discarded (%s), no slave process", s)
	}
}
...
func (mp *master) sendSignal(s os.Signal) {
	if mp.slaveCmd != nil && mp.slaveCmd.Process != nil {
		if err := mp.slaveCmd.Process.Signal(s); err != nil {
			mp.debug("signal failed (%s), assuming slave process died unexpectedly", err)
			os.Exit(1)
		}
	}
}
...
func (mp *master) fetch() {
	...
	if mp.printCheckUpdate {
		mp.debug("checking for updates...")
	}
	reader, err := mp.Fetcher.Fetch()
	if err != nil {
		mp.debug("failed to get latest version: %s", err)
		return
	}
	if reader == nil {
		if mp.printCheckUpdate {
	...
	if err != nil {
		mp.warn("failed to open temp binary: %s", err)
		return
	}
	defer func() {
		tmpBin.Close()
	...
	if err != nil {
		mp.warn("failed to write temp binary: %s", err)
		return
	}
	//compare hash
	newHash := hash.Sum(nil)
	if bytes.Equal(mp.binHash, newHash) {
		mp.debug("hash match - skip")
		return
	}
	//copy permissions
	if err := executor.Chmod(tmpBin, mp.binPerms); err != nil {
		mp.warn("failed to make temp binary executable: %s", err)
		return
	}
	if err := executor.Chown(tmpBin, executor.Uid, executor.Gid); err != nil {
		mp.warn("failed to change owner of binary: %s", err)
		return
	}
	if _, err := tmpBin.Stat(); err != nil {
		mp.warn("failed to stat temp binary: %s", err)
		return
	}
	tmpBin.Close()
	if _, err := os.Stat(tmpBinPath); err != nil {
		mp.warn("failed to stat temp binary by path: %s", err)
		return
	}
	if mp.Config.PreUpgrade != nil {
		if err := mp.Config.PreUpgrade(tmpBinPath); err != nil {
			mp.warn("user cancelled upgrade: %s", err)
			return
		}
	}
	...
	go func() {
		...
		if !returned {
			mp.warn("sanity check against fetched executable timed-out, check overseer is running")
			if cmd.Process != nil {
				cmd.Process.Kill()
			}
		}
	...
	returned = true
	if err != nil {
		mp.warn("failed to run temp binary: %s (%s) output \"%s\"", err, tmpBinPath, tokenOut)
		return
	}
	if tokenIn != string(tokenOut) {
		mp.warn("sanity check failed")
		return
	}
	//overwrite!
	if err := executor.Move(mp.binPath, tmpBinPath); err != nil {
		mp.warn("failed to overwrite binary: %s", err)
		return
	}
	mp.debug("upgraded binary (%x -> %x)", mp.binHash[:12], newHash[:12])
	mp.binHash = newHash
...
func (mp *master) triggerRestart() {
	if mp.restarting {
		mp.debug("already graceful restarting")
		return //skip
	} else if mp.slaveCmd == nil || mp.restarting {
		mp.debug("no slave process")
		return //skip
	}
	mp.debug("graceful restart triggered")
	mp.restarting = true
...
func (mp *master) fork() error {
	mp.debug("starting %s", mp.binPath)
	...
	case err := <-cmdwait:
		...
		mp.debug("prog exited with %d", code)
...
func (mp *master) debug(f string, args ...interface{}) {
	if mp.Config.Debug {
		log.Printf("[master] "+f, args...)
	}
}

func (mp *master) warn(f string, args ...interface{}) {
