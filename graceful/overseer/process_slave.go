...
func (sp *slave) watchSignal() {
	...
	go func() {
		<-signals
		signal.Stop(signals)
		sp.debug("graceful shutdown requested")
		...
		if len(sp.listeners) > 0 {
			...
			if !sp.NoRestart {
				sp.masterProc.Signal(executor.SIGUSR1)
			}
		...
		go func() {
			...
			sp.debug("timeout. forceful shutdown")
			os.Exit(1)
		}()
	}()
}
...
func (sp *slave) debug(f string, args ...interface{}) {
	if sp.Config.Debug {
		log.Printf("[slave#"+sp.id+"] "+f, args...)
	}
}

func (sp *slave) warn(f string, args ...interface{}) {
